        ...
        <header>
            <Container>
                <div className="flex flex-col items-center sm:flex-row bg-background lg:gap-4">
                    <div className="self-start inline-flex cursor-pointer items-center px-1 text-2xl lg:text-3xl font-bold text-neutral-800 py-2 mt-4 sm:my-8 sm:pr-8">OMDB</div>
                    <div className="flex gap-2 lg:gap-4">
                        <NavItem title="Home" href="/" icon={HomeIcon} className="inline-flex sm:hidden"/>
...
function NavItem({title, href, icon, className}: NavItem){
    ...
    return (
        <Link href={href} 
           ...
           <Icon className="h-6 sm:h-4 lg:h-6 lg:mr-2"/> <span className={cn(
                isActive ? "border-neutral-800 sm:border-b" : "sm:border-b sm:border-transparent sm:hover:border-neutral-600 hover:text-neutral-600  "
            )}>{title}</span>
